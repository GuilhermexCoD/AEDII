...
class ArvoreTrie {
    ...
    public void merge(ArvoreTrie x) {
        merge("", x.raiz);
    }
    ...
    public void merge(String s, No no) {
        if (no.folha == true) {
            this.inserir(s += no.elemento);
        } else {
            for (int i = 0; i < no.tamanho; i++) {
                if (no.prox[i] != null) {
                    merge(s + (no.elemento == '-' ? "" : no.elemento), no.prox[i]);
                }
            }
        }
    }
...
class TP04Q06_ArvoreTrie {
    ...
    public static void main(String[] args) throws Exception {
        ...
        Jogador[] jogadores = new Jogador[550];

        ArvoreTrie arvoreTrie1 = new ArvoreTrie();
        ArvoreTrie arvoreTrie2 = new ArvoreTrie();
        ...
        while (!isFim(line)) {
            ...
            arvoreTrie1.inserir(tmp.getNome());

            MyIO.println(tmp.getId() + "\t" + tmp.getNome());
        ...
        arvoreTrie2.merge(arvoreTrie1, arvoreTrie2);
