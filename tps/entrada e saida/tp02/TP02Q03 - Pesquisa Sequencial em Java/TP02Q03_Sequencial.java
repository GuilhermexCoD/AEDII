    ...
    */ 
    ...
    public static void main(String[] args){
        ...
        for(int i = 0; i < numEntrada; i++){
            ...
            String nomeJogador = entrada[i];
            
            MyIO.println(String.format("%s",pesqSeq(jogadores,nomeJogador)?"SIM":"NAO"));
   
        }

        long tempoFinal = System.currentTimeMillis();

        String result = (String.format("718116\t%.3f\t%d",(tempoFinal-tempoInicial)/1000d,comparacion));

        Arq.openWriteClose("718116_sequencial.txt",result);

    }

    public static boolean pesqSeq(Jogador[] jogadores, String name){
        boolean resp = false;
        int n = jogadores.length;

        for(int i = 0; i < n; i++){
            String actualName = jogadores[i].getNome();
            int actualNameLength = actualName.length();
            
            int targetNameLength = name.length();
            
            String actualCompactName = actualName.substring(0,Math.min(actualNameLength,targetNameLength));
            boolean isEqual = name.equals(actualCompactName);
            
            //MyIO.println(String.format("%s == %s (%b)",name,actualCompactName,isEqual));
            comparacion ++;
            if(isEqual){
                resp = true;
                i = n;
