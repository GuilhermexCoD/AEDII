...
class TP02Q09_Heapsort{
    ...
    */ 
    ...
    public static void main(String[] args){
        ...
        Arq.openWriteClose("718116_heapsort.txt",result);
        
    }

    public static boolean hasBiggerHeight(Jogador a, Jogador b){
    ...
    public static boolean hasLowerHeight(Jogador a, Jogador b){

        if(a.getAltura() == b.getAltura()){
            return isNameLower(a,b);
        }
        
        return a.getAltura() < b.getAltura();
    }

    public static boolean isNameLower(Jogador a,Jogador b){
        boolean result = false;
        String nomeMenor = a.getNome();
        String nomeMaior = b.getNome();
        int isMore = nomeMenor.compareTo(nomeMaior);
    ...
    public static Jogador[] sort(Jogador[] jogadores) {
        ...
        // MyIO.println("_________________ Jogador + 1 - Inicio ______________________");
        // printJogadores(jogadores);        
        // MyIO.println("_________________ Jogador + 1 - FIM ______________________");

        //Contrucao do heap
        for(int tamHeap = 2; tamHeap <= n; tamHeap++){
            jogadores = construir(jogadores,tamHeap);
        }

        // MyIO.println("_________________ Construir - Inicio ______________________");
        // printJogadores(jogadores);        
        // MyIO.println("_________________ Construir - FIM ______________________");

        //Ordenacao propriamente dita
        int tamHeap = n;
        while(tamHeap > 1){
            jogadores = swap(jogadores,1, tamHeap--);
            jogadores = reconstruir(jogadores,tamHeap);
        }

        // MyIO.println("_________________ Reconstruir - Inicio ______________________");
        // printJogadores(jogadores);        
        // MyIO.println("_________________ Reconstruir - FIM ______________________");
        ...
        // MyIO.println("_________________ Sorted - Inicio ______________________");
        //printJogadores(jogadores);        
        // MyIO.println("_________________ Sorted - FIM ______________________");

        return jogadores;
    }
  
    public static Jogador[] construir(Jogador[] jogadores,int tamHeap){
        for(int i = tamHeap; i > 1 && hasBiggerHeight(jogadores[i],jogadores[i/2]); i /= 2){
            jogadores = swap(jogadores,i, i/2);
        }

        return jogadores;
    }
  
    public static Jogador[] reconstruir(Jogador[] jogadores,int tamHeap){
        int i = 1;
        while(i <= (tamHeap/2)){
            int filho = getMaiorFilho(jogadores,i, tamHeap);
            if(hasLowerHeight(jogadores[i], jogadores[filho])){
                jogadores = swap(jogadores,i, filho);
                i = filho;
            }else{
                i = tamHeap;
            }
    ...
    public static int getMaiorFilho(Jogador[] jogadores,int i, int tamHeap){
        int filho;
        if (2*i == tamHeap || hasBiggerHeight(jogadores[2*i], jogadores[2*i+1])){
           filho = 2*i;
        } else {
           filho = 2*i + 1;
        }
    ...
    public static Jogador[] swap(Jogador[] jogadores,int i, int j) {
