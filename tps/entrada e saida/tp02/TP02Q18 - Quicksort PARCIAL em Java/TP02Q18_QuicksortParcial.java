...
class TP02Q18_QuicksortParcial{
    ...
    */ 
    ...
    public static void printJogadores(Jogador[] jogadores,int k){
        for (int i = 0; i < k; i++) {
            if(jogadores[i] != null){
                jogadores[i].imprimir();
            }
    ...
    public static void sortQuicksort(int k) {
	...
	*/
    public static void quicksort(int esq, int dir,int k) {
        ...
        while (i <= j) {
            while (compareJogador(jogadores[i],pivo) < 0) i++;
            while (compareJogador(jogadores[j],pivo) > 0) j--;
    ...
    public static int compareJogador(Jogador a , Jogador b){
        int value = 0;

        if(a.getEstadoNascimento().compareTo(b.getEstadoNascimento()) == 0){
            value = a.getNome().compareTo(b.getNome());
        }else{
            value = a.getEstadoNascimento().compareTo(b.getEstadoNascimento());
        }
