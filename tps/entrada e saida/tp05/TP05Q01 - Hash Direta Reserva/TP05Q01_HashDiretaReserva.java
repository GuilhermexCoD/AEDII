...
class Hash {
    Jogador tabela[];
    int memoriaPrincipal, memoriaReserva, memoriaTotal, reservaCount;
    ...
    public Hash(int memoriaPrincipal, int memoriaReserva) {
        ...
        this.tabela = new Jogador[this.memoriaTotal];
    ...
    public boolean inserir(Jogador elemento) {
        ...
        if (elemento != null) {
            ...
            } else if (reservaCount < memoriaReserva) {
                tabela[memoriaPrincipal + reservaCount] = Jogador.clone(elemento);
                reservaCount++;
                resp = true;
            }
        }
    ...
    public boolean pesquisar(Jogador elemento) {
        ...
        if (tabela[pos] != null && tabela[pos].getNome().compareTo(elemento.getNome()) == 0) {
            ...
            for (int i = 0; i < reservaCount; i++) {
                Jogador tmp = tabela[memoriaPrincipal + i];
                if (tmp != null && tmp.getNome().compareTo(elemento.getNome()) == 0) {
                    resp = true;
                    i = reservaCount;
                }
            }
...
class TP05Q01_HashDiretaReserva {
